function [ m ys xs ] = funWindowShape1Dbck( n, p, h )
%funWindowShape1Dbck is a support function for inverse oplWindow1D* operators.
%   [ M YS XS ] = funWindowShape1Dbck( N, P, H )
%   INPUT:
...
%           of the input vector in every window
    ...
    t=floor(n/(p*2));
    assert(h<t,'funWindowShape1Dbck: half-overlap (%d) too large for local window size (%d). Args: (%d,%d,%d)\n',h,2*t,n,p,h);
    ...
    cn=sum(xs(:,2));
    assert(cn==n,'funWindowShape1Dbck: window sum %d != n=%d',cn,n);
    cm=sum(ys(:,2));
    assert(cm==m,'funWindowShape1Dbck: reduced window sum %d != m=%d',cm,m);

    % debuging only
    %disp('funWindowShape1Dbck');
