...
switch s.type
    case {'.'}
        ...
        if strcmp(s.subs,'data')
            error('Access of out-of-core data by dot notation not allowed');
        end
    ...
    case {'()'}
        if strcmp(s.subs,':')
            ...
            if length(s.subs) == length(x.odims)
                assert(isnumeric(s.subs{end}),...
                    'Last dimension index must be numeric')
                ind    = s.subs{end};
                s.subs = s.subs(1:end-1);
                
                if ind == x.iSlice
                    varargout{1} = subsref(x.inCore,s);
                else
                    varargout{1} = subsref(getSlice(x,ind),s);
                end
            else % Normal subsref
